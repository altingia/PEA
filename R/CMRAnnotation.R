...
.getUTR <- function(curID, GTF){
  ...
  curIDRange <- subset(GTF, GTF$V3 == "transcript" & GTF$V6 == curID)[,4:5]
  ...
  cds.len <- curIDRange$V5 - curIDRange$V4 + 1 - len.5 - len.3
  if(len.5 != 0){
    UTR5Range <- c(1, len.5)
  ...
  if(len.3 != 0){
    UTR3Range <- c((cds.len + len.5 + 1), (curIDRange[2] - curIDRange[1] + 1))
  }else{
    UTR3Range <- c(0, 0)
  }
...
getUTR <- function(GTF){
  ...
  curGTF <- subset(GTF, GTF$V3 == "transcript" | GTF$V3 == "five_prime_utr" | GTF$V3 == "three_prime_utr")
  curGTF$V6 <- sapply(curGTF$V9, 
