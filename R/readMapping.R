...
readMapping <- function(alignment = c("bowtie", "bowtie2", "tophat", 
  ...
  cat("This function is used for aligning reads to the reference genome using", "\n",
      "bowtie, bowtie2, tophat, tophat2, hisat or hisat2, and can only be used in 
      Linux operation system.", "Would you like to continue? (Y/N)", "\n")
  ...
  if(x == "y" | x == "Y"){
    if(length(alignment) > 1){
      ...
      cat("The length of alignmen > 1, the first one will be used!", "\n")
    }
    ...
    if(!paired){
      ...
      if(length(fq) != 2){
        stop("Pair-end alignment should provide two fastq files!")
      }
